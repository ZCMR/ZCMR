...
class SentenceDataset(Dataset):
    ...
    def __init__(self, fname, tokenizer, opt, vocab_help):
        ...
        for obj in tqdm(parse(fname), total=len(parse(fname)), desc="Training examples"):
            ...
            adj_1 = np.ones(opt.max_length) * opt.pad_id
            adj_2 = np.ones(opt.max_length) * opt.pad_id
            # print('194',adj.shape)
            if opt.parseadj:
                ...
                adj = idx2gragh[num]
                adj_1 = idx2gragh_s[num]
                adj_2 = idx2gragh_sdat[num]
                num += 3
                ...
                adj_1 = np.pad(adj_1, (0, opt.max_length - adj_1.shape[0]), 'constant')
                adj_2 = np.pad(adj_2, (0, opt.max_length - adj_2.shape[0]), 'constant')
