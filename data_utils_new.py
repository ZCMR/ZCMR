    ...
    with open(data_path) as infile:
        ...
        for d in data:
            for aspect in d['aspects']:
                ...
                aspect['from'] = int(aspect['from'])
                aspect['to'] = int(aspect['to'])
                # position
                aspect_post = [aspect['from'], aspect['to']] 
                # post = [1-(aspect['from']-i)/length for i in range(aspect['from'])] \
                #        +[0 for _ in range(aspect['from'], aspect['to'])] \
                #        +[1-(i-aspect['to']+1)/length for i in range(aspect['to'], length)]
...
class SentenceDataset(Dataset):
    ...
    def __init__(self, fname, tokenizer, opt, vocab_help):
        ...
        for obj in tqdm(parse(fname), total=len(parse(fname)), desc="Training examples"):
            ...
            # print('194',adj.shape)
            if opt.parseadj:
                #fname = './DualGCN/dataset/Laptops_corenlp/'
                # fin = open(fname + '.graph', 'rb')
                # idx2gragh = pickle.load(fin)
                # fin.close()
                # fin = open(fname + '.graph_s', 'rb')
            ...
            #print('230',text.shape,adj.shape)
            data.append({
                'text': text, 
                'aspect': aspect, 
                'post': post,
...
class ABSAGCNData(Dataset):
    def __init__(self, fname, tokenizer, opt):
        ...
        for obj in tqdm(parse(fname), total=len(parse(fname)), desc="Training examples"):
            ...
            fin.close()
            sdat_graph = np.pad(idx2graph_sdat[num], \
                ((0,tokenizer.max_seq_len-idx2graph_sdat[num].shape[0]),(0,tokenizer.max_seq_len-idx2graph_sdat[num].shape[0])), 'constant')
            num += 1
            post = np.pad(obj['post'],(0,tokenizer.max_seq_len-len(obj['post'])),'constant')
