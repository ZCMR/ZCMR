...
class GCNAbsaModel(nn.Module):
    ...
    def forward(self, inputs):
        ...
        #     return adj.cuda()
        # adj_deps = inputs_to_tree_reps(head.data, tok.data, l.data)
        #adj1,adj2,adj3 = getadj()
        #h1, h2, adj_ag = self.gcn(adj_dep, inputs,adj_deps)
        h1, h2, adj_ag = self.gcn(adj_dep, inputs,adj_dep)
        # avg pooling asp feature
        # asp_wn = mask.sum(dim=1).unsqueeze(-1)                        # aspect words num
        # mask = mask.unsqueeze(-1).repeat(1,1,self.opt.hidden_dim)     # mask for h
        # h1 = (h1*mask).sum(dim=1) / asp_wn
        # h2 = (h2*mask).sum(dim=1) / asp_wn
        # print(outputs1.shape,outputs2.shape)
        return h1, h2, adj_ag, adj_dep
...
class GCN(nn.Module):
    def __init__(self, opt, embeddings, mem_dim, num_layers):
        ...
        self.gcn_drop_1 = nn.Dropout(opt.gcn_dropout1)
        ...
        self.W4 = nn.ModuleList()
        for layer in range(self.layers):
            input_dim = self.in_dim if layer == 0 else self.mem_dim
            self.W.append(nn.Linear(input_dim, self.mem_dim))
            self.W1.append(nn.Linear(mem_dim, self.mem_dim))
            self.W2.append(nn.Linear(mem_dim, self.mem_dim))
            self.W3.append(nn.Linear(mem_dim, self.mem_dim))
            self.W4.append(nn.Linear(mem_dim, self.mem_dim))
        ...
        self.classifier = nn.Linear(self.mem_dim * 2, self.mem_dim)
    ...
    def forward(self, adj, inputs,adj_deps):
        ...
        adj_ag =  torch.zeros_like(attn_adj_list[0])
        ...
        for j in range(attn_adj_list[0].size(0)):
        ...
        for l in range(self.layers):
            ...
            outputs_dep = self.gcn_drop_1(gAxW_dep) if l < self.layers-1 else gAxW_dep
            outputs_ag = self.gcn_drop_1(gAxW_ag) if l < self.layers-1 else gAxW_ag

        outputs_deps = outputs_dep
        outputs_ags = outputs_ag

        for l in range(self.layers):
            # **********
            # **SynGCN*************
            Ax_dep = adj.bmm(outputs_dep)
            AxW_dep = self.W4[l](Ax_dep)
            AxW_dep = AxW_dep / denom_dep
            gAxW_dep = F.relu(AxW_dep)

            # Ax_dep = adj.bmm(gAxW_dep)
            # AxW_dep = self.W2[l](Ax_dep)
            # AxW_dep = AxW_dep / denom_dep
            # gAxW_dep = F.relu(AxW_dep)

            # ************SemGCN*************
            Ax_ag = adj_ag.bmm(outputs_ag)
            AxW_ag = self.W3[l](Ax_ag)
            AxW_ag = AxW_ag / denom_ag
            gAxW_ag = F.relu(AxW_ag)
        ...
        outputs_deps = outputs_dep + outputs_deps
        outputs_ags = outputs_ag + outputs_ags

        x = mask * outputs_deps
        y = mask * outputs_ags
        gcn_inputs = self.classifier(gcn_inputs)
        # print('266',x.shape,y.shape,mask.shape,gcn_inputs.shape)
        alpha_mat = torch.matmul(x, gcn_inputs.transpose(1, 2))
        alpha = F.softmax(alpha_mat.sum(1, keepdim=True), dim=2)
        x = torch.matmul(alpha, x).squeeze(1) # batch_size x 2*hidden_dim
        # print('270',alpha_mat.shape,alpha.shape,x.shape)
        alpha_mat_y = torch.matmul(y, gcn_inputs.transpose(1, 2))
        alpha_y = F.softmax(alpha_mat_y.sum(1, keepdim=True), dim=2)
        y = torch.matmul(alpha_y, y).squeeze(1) # batch_size x 2*hidden_dim
        # print('273',x.shape,y.shape,mask.shape)
        #return outputs_deps,outputs_ags,adj_ag
        return x, y, adj_ag
